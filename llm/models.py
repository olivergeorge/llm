...
class _BaseResponse:
    ...
    def _apply_replay(self, replayed: Any) -> None:
        """Hydrate state from a replay store hit.

        Sets chunks, response_json, replay flags, optional tool_calls /
        tool_results / resolved_model, appends to conversation, and marks the
        response done. Callers are responsible for invoking the done callback
        themselves (sync: ``self._on_done()``; async: ``await self._on_done()``).
        """
